...
Количество операциq для сортировки = O(nˆ2) -- сложность.
Определяется верхняя оценка - не хуже чем.
асимптотика по скорости обращения
игнорируем требования памяти

Сортировка вставкой = insert sort.
Метафора: солдаты прибегают по-одному,
каждый раз массив пересортировывается.

Сортировка методом выбора = choise sort.
Описание: ищем мин. (или макс.) элемент, 
ставим его на соответствующее место.
Метафора: мин = постамент == сравниваем каждый и меняем местами!
инвариант цикла
